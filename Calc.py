...
Label(window, textvariable=calc_input_text).grid(row=1, column=2)
result_text = StringVar()
Label(window, textvariable=result_text).grid(row=0, column=2)
Button(window, text="Fermer", command=window.quit).grid(row=0, column=3)
Button(window, text=" 0 ", command=lambda: input_key("0")).grid(row=6, column=1)
